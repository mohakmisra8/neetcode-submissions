class Solution {
    public double[] calcEquation(
        ...
        // iterate over equations and add to map
        for (int i = 0; i < equations.size(); i++) {
            String a = equations.get(i).get(0);
            String b = equations.get(i).get(1);
            double v = values[i];
            // get a's neighbor map (make an empty one if it doesn't exist yet),
        ...
        for (int i = 0; i< queries.size(); i++) {
            ...
            res[i] = dfs(c, d, new HashSet<>(), graph);
        }
        return res;
